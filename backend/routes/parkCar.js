...
router.post('/', (req, res) => {
    ...
    if (!req.body) { console.log('Body error'); res.status(404).json({message:'body error'})}
        ...
        console.log({body});
        db.all("select id from Space where id not in (select Space_id from Parking_stand where datetime('now') between time_from and time_to or datetime('now','+"+days+" days','+"+hours+" hours','+"+seconds+" seconds') between time_from and time_to) and Parking_id=(select id from Parking where adress=(?))",
        [adress], (err,rows)=>{
            if(err){
