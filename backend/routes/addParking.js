...
router.post('/', (req, res) => {
    if (!req.body) { console.log('Body error:\n'); res.sendStatus(400) } else {
        db.get('select * from Parking where adress=(?)',[req.body.adress],(err,row)=>{
            if(row!=undefined){res.status(200).json({message:'Parking exists'});console.log('SUKA'); return}
            else{
                db.run('Insert into Parking values(null, ?)',[req.body.adress], (err)=>{
